...
class TestCovarianceTypes:
    ...
    @pytest.mark.parametrize(
        "cov_type",
        ["nonrobust", "robust", "hc0", "hc2", "hc3", "clustered", "twoway"],
    )
    ...
    def test_pcse(self, balanced_panel_data):
        """Test panel-corrected standard errors (may produce NaN with T < N)."""
        model = PooledOLS("y ~ x1 + x2", balanced_panel_data, "entity", "time")
    ...
    @pytest.mark.parametrize("cov_type", ["driscoll_kraay", "newey_west"])
    def test_hac_se_with_lags(self, balanced_panel_data, cov_type):
        """Test HAC standard errors with explicit max_lags."""
        model = PooledOLS("y ~ x1 + x2", balanced_panel_data, "entity", "time")
        results = model.fit(cov_type=cov_type, max_lags=2)
    ...
    @pytest.mark.parametrize("cov_type", ["driscoll_kraay", "newey_west"])
    def test_hac_se_with_kernel(self, balanced_panel_data, cov_type):
        """Test HAC standard errors with kernel option."""
        model = PooledOLS("y ~ x1 + x2", balanced_panel_data, "entity", "time")
        results = model.fit(cov_type=cov_type, max_lags=3, kernel="bartlett")
    ...
    @pytest.mark.parametrize("cov_type", ["driscoll_kraay", "newey_west"])
    def test_hac_se_no_lags(self, balanced_panel_data, cov_type):
        """Test HAC standard errors without specifying max_lags."""
        model = PooledOLS("y ~ x1 + x2", balanced_panel_data, "entity", "time")
        results = model.fit(cov_type=cov_type)
...
class TestPrivateMethods:
    ...
    def test_compute_vcov_robust(self, balanced_panel_data):
        ...
        _beta, resid, _fitted = compute_ols(y, X, None)
