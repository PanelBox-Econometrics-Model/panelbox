...
def sample_panel_dataframe():
    ...
    index = pd.MultiIndex.from_product([entities, periods], names=["entity", "time"])
...
def unbalanced_panel_dataframe():
    ...
    for entity in range(1, 6):
        ...
        for t in range(n_periods):
            data.append({"entity": f"E{entity:02d}", "time": 2000 + t, "value": np.random.randn()})
...
def mock_panel_results():
    ...
    mock.entity_effects = pd.Series(
        data=np.random.randn(5) * 0.5, index=["A", "B", "C", "D", "E"], name="effects"
    )

    # Mock standard errors
    mock.std_errors = pd.Series(
        data=np.random.uniform(0.1, 0.3, 5), index=["A", "B", "C", "D", "E"], name="std_errors"
    )

    # Mock time effects
    mock.time_effects = pd.Series(
        data=np.random.randn(10) * 0.3, index=list(range(2000, 2010)), name="time_effects"
    )
...
class TestCalculateBetweenWithin:
    ...
    def test_with_dataframe_subset_variables(self, sample_panel_dataframe):
        ...
        result = PanelDataTransformer.calculate_between_within(
            sample_panel_dataframe, variables=["var1", "var2"]
        )
    ...
    def test_with_dict_data(self):
        ...
        dict_data = {"variables": ["x", "y"], "between_var": [1.0, 2.0], "within_var": [0.5, 1.5]}
    ...
    def test_single_variable(self, sample_panel_dataframe):
        ...
        result = PanelDataTransformer.calculate_between_within(
            sample_panel_dataframe, variables=["var1"]
        )
    ...
    def test_unbalanced_panel(self, unbalanced_panel_dataframe):
        ...
        result = PanelDataTransformer.calculate_between_within(unbalanced_panel_dataframe)
    ...
    def test_invalid_variable_name(self, sample_panel_dataframe):
        ...
        result = PanelDataTransformer.calculate_between_within(
            sample_panel_dataframe, variables=["nonexistent_var"]
        )
...
class TestAnalyzePanelStructure:
    ...
    def test_single_entity(self):
        ...
        data = pd.DataFrame(
            {"value": [1, 2, 3]},
            index=pd.MultiIndex.from_product([["A"], [2000, 2001, 2002]], names=["entity", "time"]),
        )
    ...
    def test_single_period(self):
        ...
        data = pd.DataFrame(
            {"value": [1, 2, 3]},
            index=pd.MultiIndex.from_product([["A", "B", "C"], [2000]], names=["entity", "time"]),
        )
...
class TestPanelDataTransformerIntegration:
    ...
    def test_all_methods_with_dataframe(self, sample_panel_dataframe):
        ...
        # Between-within
        bw_result = PanelDataTransformer.calculate_between_within(sample_panel_dataframe)
        assert bw_result is not None

        # Panel structure
        ps_result = PanelDataTransformer.analyze_panel_structure(sample_panel_dataframe)
    ...
    def test_consistency_across_methods(self, sample_panel_dataframe):
        ...
        # Get panel structure
        PanelDataTransformer.analyze_panel_structure(sample_panel_dataframe)
...
class TestPanelDataTransformerEdgeCases:
    ...
    def test_empty_dataframe(self):
        ...
        df = pd.DataFrame(
            columns=["value"], index=pd.MultiIndex.from_tuples([], names=["entity", "time"])
        )
    ...
    def test_constant_variable(self):
        ...
        index = pd.MultiIndex.from_product(
            [[f"E{i}" for i in range(n_entities)], list(range(n_periods))], names=["entity", "time"]
        )

        df = pd.DataFrame({"constant": [5.0] * (n_entities * n_periods)}, index=index)
