...
class TestPrepareCoefficients:
    ...
    def test_prepare_coefficient_comparison_simple(self, mock_results_simple):
        ...
        # Check structure - coefficients and std_errors are dicts
        assert len(data["models"]) == 2
        assert isinstance(data["coefficients"], dict)
        assert isinstance(data["std_errors"], dict)
        assert len(data["coefficients"]) == 3  # x1, x2, const
        assert len(data["std_errors"]) == 3
    ...
    def test_prepare_coefficient_comparison_subset_variables(self, mock_results_simple):
        ...
        transformer = ComparisonDataTransformer()
        variables = ["x1", "x2"]
        data = transformer.prepare_coefficient_comparison(mock_results_simple, variables=variables)

        assert set(data["coefficients"].keys()) == set(variables)
        assert len(data["coefficients"]) == 2  # 2 variables
...
class TestPrepareForestPlot:
    ...
    def test_prepare_forest_plot_confidence_level(self, mock_results_simple):
        ...
        # 95% CI
        data_95 = transformer.prepare_forest_plot(mock_results_simple[0], confidence_level=0.95)

        # 99% CI
        data_99 = transformer.prepare_forest_plot(mock_results_simple[0], confidence_level=0.99)
    ...
    def test_prepare_forest_plot_subset_variables(self, mock_results_simple):
        ...
        transformer = ComparisonDataTransformer()
        variables = ["x1"]
        data = transformer.prepare_forest_plot(mock_results_simple[0], variables=variables)
