...
class SpatialPanelModel(PanelModel):
    ...
    def _within_transformation(
        ...
        if isinstance(X, np.ndarray):
            ...
            if X.ndim == 1:
                X = pd.Series(X, index=self.data.index)
            else:
                X = pd.DataFrame(X, index=self.data.index)

        # Demean by entity
        X_demeaned = X.groupby(level=self.entity_col).transform(lambda x: x - x.mean())

        if isinstance(X_demeaned, pd.Series):
