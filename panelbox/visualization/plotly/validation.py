...
class TestOverviewChart(PlotlyChartBase):
    ...
    def _create_figure(self, data: Dict[str, Any], **kwargs) -> go.Figure:
        ...
        # Passed trace (green)
        passed_text = [f"{p} ({pct:.1f}%)" if show_percentages else str(p)
        ...
        # Failed trace (red)
        failed_text = [f"{f} ({pct:.1f}%)" if show_percentages else str(f)
...
class TestStatisticsChart(PlotlyChartBase):
    ...
    def _create_figure(self, data: Dict[str, Any], **kwargs) -> go.Figure:
        ...
        category_colors = {cat: self.theme.get_color(i)
...
class ValidationDashboard(PlotlyChartBase):
    ...
    def _create_figure(self, data: Dict[str, Any], **kwargs) -> go.Figure:
        ...
        fig.add_trace(
            go.Bar(x=test_names, y=pvalues, marker_color=colors, showlegend=False),
            row=1, col=2
        )
        ...
        fig.add_trace(
            go.Indicator(
                ...
                gauge={
                    'axis': {'range': [0, 100]},
                    'bar': {'color': self.theme.success_color if pass_rate >= 80 else self.theme.warning_color},
                    'steps': [
