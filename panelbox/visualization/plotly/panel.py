...
class EntityEffectsPlot(PlotlyChartBase):
    ...
    def _create_figure(self, data: Dict[str, Any], **kwargs) -> go.Figure:
        ...
        if max_entities and len(df) > max_entities:
            ...
            df = pd.concat(
                [df_sorted.head(n_half), df_sorted.tail(max_entities - n_half)]
        ...
        fig.add_trace(
            go.Bar(
                ...
                textposition="auto",
                hovertemplate=("<b>%{y}</b><br>" + "Effect: %{x:.4f}<br>" + "<extra></extra>"),
            )
        )
...
class TimeEffectsPlot(PlotlyChartBase):
    ...
    def _create_figure(self, data: Dict[str, Any], **kwargs) -> go.Figure:
        ...
        # Get config
        show_confidence = kwargs.get("show_confidence", self.config.get("show_confidence", True))
        highlight_significant = kwargs.get(
            "highlight_significant", self.config.get("highlight_significant", True)
        )
        significance_level = kwargs.get(
            "significance_level", self.config.get("significance_level", 0.05)
        )
...
class BetweenWithinPlot(PlotlyChartBase):
    ...
    def _create_figure(self, data: Dict[str, Any], **kwargs) -> go.Figure:
        ...
        style = kwargs.get(
            "chart_type",
            kwargs.get("style", self.config.get("chart_type", self.config.get("style", "stacked"))),
        )
        ...
        if style == "stacked":
            fig = self._create_stacked_chart(
                variables, between_var, within_var, between_pct, within_pct, show_percentages
            )
            ...
            fig = self._create_scatter_chart(
                variables, between_var, within_var, between_pct, within_pct
            )
    ...
    def _create_stacked_chart(
        self, variables, between_var, within_var, between_pct, within_pct, show_percentages
    ):
        ...
        # Between variance
        text_between = [
            f"{p:.1f}%" if show_percentages else f"{v:.2f}"
        ...
        # Within variance
        text_within = [
            f"{p:.1f}%" if show_percentages else f"{v:.2f}" for p, v in zip(within_pct, within_var)
        ]
        ...
        fig.update_layout(barmode="stack", xaxis_title="Variable", yaxis_title="Variance")

        return fig

    def _create_side_by_side_chart(
        self, variables, between_var, within_var, between_pct, within_pct, show_percentages
    ):
        ...
        fig.add_trace(
            go.Bar(
                ...
                name="Between Variance",
                marker_color=self.theme.get_color(0),  # Primary
                text=[f"{p:.1f}%" if show_percentages else "" for p in between_pct],
        ...
        fig.add_trace(
            go.Bar(
                ...
                name="Within Variance",
                marker_color=self.theme.get_color(1),  # Secondary
                text=[f"{p:.1f}%" if show_percentages else "" for p in within_pct],
                textposition="outside",
            )
        )

        fig.update_layout(barmode="group", xaxis_title="Variable", yaxis_title="Variance")

        return fig

    def _create_scatter_chart(self, variables, between_var, within_var, between_pct, within_pct):
        ...
        fig.update_layout(xaxis_title="Between Variance", yaxis_title="Within Variance")
