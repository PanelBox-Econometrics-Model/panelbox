...
from panelbox.diagnostics.spatial_tests import MoranIPanelTest, run_lm_tests
...
def spatial_test_data():
    ...
    with open(FIXTURES_PATH / "true_params.json") as f:
...
def r_validation():
    ...
    with open(FIXTURES_PATH / "r_complete_validation.json") as f:
        return json.load(f)
...
class TestCompleteValidation:
    ...
    def test_morans_i_complete(self, spatial_test_data, r_validation):
        ...
        assert np.isclose(result_pooled.statistic, r_stat, rtol=0.15), (
    ...
    def test_sar_re_complete(self, spatial_test_data, r_validation):
        ...
        for var_name, r_beta_list in r_sar_re["beta"].items():
            ...
            assert np.isclose(py_beta, r_beta, rtol=0.15), (
...
class TestParameterRecovery:
    ...
    def test_recover_rho(self, spatial_test_data):
        ...
        assert np.abs(result.rho - true_rho) < 0.15, (
    ...
    def test_recover_beta(self, spatial_test_data):
        ...
        for i in range(3):
            ...
            assert np.abs(recovered_beta[i] - true_beta[i]) < 0.30, (
