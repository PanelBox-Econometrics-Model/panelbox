...
def test_irf_h0_equals_cholesky_diagonal(simple_var_data):
    ...
    for i in range(2):
        assert np.isclose(irf_result.irf_matrix[0, i, i], P[i, i], atol=1e-6), (
...
def test_irf_cumulative(simple_var_data):
    ...
    assert np.allclose(irf_cumulative.irf_matrix, manual_cumsum, atol=1e-10), (
...
def test_irf_ordering_affects_cholesky(simple_var_data):
    ...
    assert not np.allclose(irf_order1.irf_matrix, irf_order2.irf_matrix, atol=1e-6), (
...
def test_irf_shock_size(simple_var_data):
    ...
    assert np.allclose(irf_2x.irf_matrix, 2.0 * irf_1std.irf_matrix, atol=1e-10), (
...
def test_irf_recursive_vs_companion(simple_var_data):
    ...
    # Should be identical
    assert np.allclose(Phi_recursive, Phi_companion, atol=1e-8), (
