...
def test_girf_order_invariance(simple_var_data):
    ...
    assert np.allclose(girf_order1.irf_matrix, girf_reordered, atol=1e-8), (
...
def test_girf_equals_cholesky_for_diagonal_sigma(diagonal_var_data):
    ...
    df, _A1_true, _Sigma_true = diagonal_var_data
    ...
    assert np.allclose(chol_irf.irf_matrix, girf.irf_matrix, atol=0.05), (
...
def test_girf_differs_from_cholesky_for_nondiagonal(simple_var_data):
    ...
    # They should differ
    assert not np.allclose(chol_irf.irf_matrix, girf.irf_matrix, atol=1e-6), (
