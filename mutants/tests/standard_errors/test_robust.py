...
class TestMeatComputation:
    ...
    def test_hc_ordering(self, setup_data):
        ...
        np.sqrt(np.diag(cov_hc2))
        np.sqrt(np.diag(cov_hc3))
...
class TestRobustStandardErrors:
    ...
    @pytest.mark.parametrize(
        ("method", "has_leverage"),
        [("HC0", False), ("HC1", False), ("HC2", True), ("HC3", True)],
    )
    def test_hc_result(self, setup_regression, method, has_leverage):
        """Test HC computation for different methods."""
        X, resid, _ = setup_regression
        robust = RobustStandardErrors(X, resid)
    ...
    @pytest.mark.parametrize("method", ["HC0", "HC1", "HC2", "HC3"])
    def test_positive_standard_errors(self, setup_regression, method):
    ...
    @pytest.mark.parametrize("method", ["HC0", "HC1", "HC2", "HC3"])
    def test_symmetric_covariance(self, setup_regression, method):
