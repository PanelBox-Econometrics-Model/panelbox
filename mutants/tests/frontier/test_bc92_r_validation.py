...
def test_bc92_vs_r_reference():
    ...
    assert not np.allclose(firm1_eff, firm1_eff[0]), (
...
def test_bc92_r_validation_with_known_values():
    ...
    assert np.allclose(beta_est, beta_true, atol=0.3), (
    ...
    assert np.abs(result.sigma_v - sigma_v_true) < 0.1, "Sigma_v estimate too far from true value"

    # Sigma_u should be close
    print(f"\nTrue sigma_u: {sigma_u_true:.4f}")
    print(f"Estimated sigma_u: {result.sigma_u:.4f}")
    assert np.abs(result.sigma_u - sigma_u_true) < 0.15, "Sigma_u estimate too far from true value"

    # Eta should be close
    eta_est = result.params.iloc[-1]
    print(f"\nTrue eta: {eta_true:.4f}")
    print(f"Estimated eta: {eta_est:.4f}")
    assert np.abs(eta_est - eta_true) < 0.03, (
