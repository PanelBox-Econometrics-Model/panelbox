...
class TestSARValidation(TestValidationBase):
    ...
    def test_sar_fe_vs_true_params(self, scenario):
        ...
        # Check rho
        assert abs(result.params["rho"] - true_rho) < rho_tol, (
        ...
        for i in range(k):
            param_name = f"x{i + 1}"
            assert abs(result.params[param_name] - true_beta[i]) < beta_tol, (
...
class TestSEMValidation(TestValidationBase):
    ...
    def test_sem_fe_vs_true_params(self, scenario):
        ...
        # Check lambda
        assert abs(result.params["lambda"] - true_lambda) < lambda_tol, (
