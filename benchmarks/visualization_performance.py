...
class VisualizationBenchmark:
    ...
    def run_all(self) -> list[BenchmarkResult]:
        ...
        for chart_type in chart_types:
            ...
            for n_entities, n_periods, size_label in test_configs:
                result = self._benchmark_chart(chart_type, n_entities, n_periods, size_label)
                self.results.append(result)

                status = "✅" if result.success else "❌"
                print(
                    f"  {status} {size_label:12} ({n_entities}x{n_periods}): "
    ...
    def _benchmark_chart(
        self, chart_type: str, n_entities: int, n_periods: int, size_label: str
    ) -> BenchmarkResult:
        """
        ...
        try:
            ...
            self._create_chart(chart_type, data)
    ...
    def _generate_test_data(self, chart_type: str, n_entities: int, n_periods: int) -> dict:
    ...
    def _format_text_report(self) -> str:
        ...
        # Group by chart type
        chart_types = sorted({r.chart_type for r in self.results})
        ...
        if successful_results:
            ...
            if slow_results:
                ...
                for result in slow_results:
                    report.append(
                        f"  • {result.chart_type} ({result.data_size}): "
